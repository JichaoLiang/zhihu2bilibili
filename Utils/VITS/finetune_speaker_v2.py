  ...
  for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, speakers) in enumerate(tqdm(train_loader)):
    ...
    if rank==0:
      if global_step % hps.train.log_interval == 0:
        ...
        linfo = [x.item() for x in losses] + [global_step, lr]
        logger.info(f'loss_disc:{linfo[0]}, loss_gen:{linfo[1]}, loss_fm:{linfo[2]}, loss_mel:{linfo[3]}, loss_dur:{linfo[4]}, loss_kl:{linfo[5]}, global_step:{linfo[6]}, lr:{linfo[7]}')
